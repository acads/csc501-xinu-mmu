...
struct sd {
	...
	unsigned int	sd_iscode : 1;      /* code or data? */
	unsigned int	sd_isapp : 1;
	unsigned int	sd_dpl : 2;         /* privilege level */
	unsigned int	sd_present : 1;     /* present in memory or not */
	unsigned int	sd_hilimit : 4;
	unsigned int	sd_avl : 1;         /* available for use? */
	unsigned int	sd_mbz : 1;	    /* must be '0' */
	unsigned int	sd_32b : 1;         
	unsigned int	sd_gran : 1;        /* segsize - 1b or 4kb */
