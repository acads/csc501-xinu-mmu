					...
					/* array in the code)		*/
{
	...
	pptr->pdevs[0] = pptr->pdevs[1] = pptr->ppagedev = BADDEV;
        ...
        if (NULL == pptr->pgdir) {
            kprintf("\n\n");
            kprintf("FATAL ERROR: Ran out of free frames for page " \
                    "directories!\n");
            kprintf("Process '%s' creation will be aborted.\n", pptr->pname);
            kprintf("\n\n");
            sleep(9);
            restore(ps);
            return SYSERR;
        }
        pd_frptr = FR_GET_FPTR(FR_PA_TO_ID(pptr->pgdir));
        pd_frptr->fr_pid = pid;
        DTRACE("DBG$ %d %s> pid %d pgd at 0x%08x in fr id %d\n",    \
                currpid, __func__, pptr->pid, pptr->pgdir,          \
                FR_PA_TO_ID(pptr->pgdir));
#ifdef DBG_ON
        print_pdbr(EMPTY);
#endif
