...
*/
    ...
    /* Intialize BS tables to facilitate demand paging for PA3 */
    init_bsm();

    /* Intialize all availalbe free frames. */
    init_frm_tab();

    /* Initialize default SC page replacement algorithm. */
    sc_init_tab(TRUE);

    /* Initialize the global page tables to map whole 16 MB of PM. */
    init_global_pgt();

    /* Insall high level page fault handler. */
    set_evec(14, (unsigned) pf_interrupt);

    /* Create a pd for the NULLPROC. We can enable paging only after
    * having at least one pd from where page lookup process can be 
    * initiated.
    */
    proctab[NULLPROC].pgdir = null_pd = new_pgd();
    if (NULL == null_pd) {
        DTRACE("DBG$ %d %s> null pd for nullproc\n", currpid, __func__);
        return SYSERR;
    }
    pd_frptr = FR_PA_TO_ID(null_pd);
    pd_frptr->fr_pid = NULLPROC;
    set_pdbr((unsigned) null_pd);

    /* Since we are all set for paging, go ahead and enable it. */
    enable_paging();
